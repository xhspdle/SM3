...
<body>
	<div id="wrap">
		<div id="north">
			<div>이벤트목록</div>
			<br> <a href="<c:url value='/dev/board/EN_main.jsp'/>">메인으로</a><br>
		</div>
		<div class="box-underimage">
			...
			<c:forEach var="vo" items="${requestScope.list }">
				<br>
				<img src='<c:url value="/images/${vo.en_savimg }"/>'
...
날짜<input type="text" name="날짜" value="${vo.en_date }">
				...
				<a
					href="<c:url value='/dev/board/EventNotice.do?cmd=select&en_num=${vo.en_num }'/>">수정</a>
				<a
					href="<c:url value='/dev/board/EventNotice.do?cmd=delete&en_num=${vo.en_num }'/>">삭제</a>
				<br>
			</c:forEach>
		</div>
	</div>
	...
	%>
	<form method="post"
		action="<c:url value='/dev/board/EventNotice.do?cmd=list'/>">
		<select name="search">
	...
	<div>
		<c:choose>
			<c:when test="${startPage>3}">
				<a
					href="<c:url value ='/dev/board/EventNotice.do?cmd=list&pageNum=${startPage-1 }&search=${param.search}&keyword=${param.keyword}'/>">[이전]</a>
			</c:when>
			<c:otherwise>
		[이전]
		</c:otherwise>
		...
		<c:forEach var="i" begin="${startPage }" end="${endPage }">
			<c:choose>
				<c:when test="${i==pageNum }">
					<a
						href="<c:url value='/dev/board/EventNotice.do?cmd=list&pageNum=${i }&search=${param.search}&keyword=${param.keyword }'/>">
		...
		<c:choose>
			<c:when test="${endPage<pageCount }">
				<a
					href="<c:url value='/dev/board/EventNotice.do?cmd=list&pageNum=${endPage+1 }&search=${param.search}&keyword=${param.keyword}'/>">[다음]</a>
			</c:when>
			<c:otherwise>
		[다음]
		</c:otherwise>
