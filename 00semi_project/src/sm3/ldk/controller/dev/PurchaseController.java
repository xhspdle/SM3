...
public class PurchaseController extends HttpServlet{
	...
	protected void insert(HttpServletRequest request, 
		...
		String spur_num=request.getParameter("pur_num");//테스트용
		...
		int order_cnt=0;
		int item_price=0;
		if(spur_num!=null && !spur_num.equals("")) {//테스트용
		...
		int nn=PurchaseListDao.getInstance().insert(new PurchaseListVo(0, pur_num,
	...
	protected void list(HttpServletRequest request, 
		...
		if(list!=null) {
			...
			request.getRequestDispatcher("PURCHAE_LIST_list.jsp").forward(request, response);
		}else {
	...
	protected void select(HttpServletRequest request, 
		...
		if(vo!=null) {
			...
			request.getRequestDispatcher("PURCHAE_LIST_insert.jsp?do1=update").forward(request, response);
		}else {
